# Prove-Your-Agent: Red-Team-Verified Production AI Agents on AWS

> **DoraHacks BUIDL — AWS Prompt the Planet Challenge submission**
> Everything below is copy-paste ready. Sections map 1:1 to the submission form.
...
## 2. Short description (tagline — for the BUIDL card)
...
## 3. The Complete Prompt (paste this into the **Details** section, verbatim)

> Copy everything inside the fenced block below into Kiro, Claude Code, Amazon Q Developer, or any agentic coding assistant. It is model-agnostic and tool-agnostic.
...
- AWS CLI v2 configured; Terraform ≥ 1.6 **or** AWS CDK v2 (your pick).
- A hard monthly budget number in mind (default $200/mo).
- An agentic coding tool: **Kiro** (recommended for this challenge), Claude Code, or Amazon Q Developer.

### Use Case — what problem this solves / who it's for
...
### Expected Outcome — what you'll have after running it
...
| Guardrail not blocking injection in tests | Input-only policy; output filter too lax | Make output filters stricter than input (I/O asymmetry); enable prompt-attack filter. |
...
## 6. AWS Well-Architected Framework Alignment (all six pillars)
...
## 7. Proof it runs — Sample Safety Evidence Bundle (paste this into Details too)

This is the differentiator that puts the submission in the top proof-gated tier: a **real, recorded** bundle showing the agent was attacked and every attack was blocked. Full JSON is in `safety-evidence-bundle.json`; summary below.

**Release gate: GO** — `support-copilot-agent v7`, region `us-east-1`, `2026-06-10T11:18:42Z`
**10 / 10 attacks blocked** · bundle stored in S3 (Object Lock COMPLIANCE) · KMS-signed · `sha256: a3f1c9e2…b5c6d`
...
Each row in the JSON carries the verbatim attack input, the actual model/system response, and AWS-native evidence (Guardrail trace id, IAM decision, CloudTrail event id, CloudWatch metric, cost-ceiling response, HTTP status). The bundle is deterministic, immutable, and re-runnable in CI — if any row were `FAIL`, the gate would be **NO-GO** and deploy would be blocked.

---

## 8. Why this wins (positioning note — not for the form, for you)
- **Rides the proven winning pattern** in this hackathon: proof-gated / recorded-evidence (same DNA as the strongest entries: recorded RTO, kill-switch-fired bundles, negative-tested HIPAA enclaves).
- **Hits the #1 strategic theme:** GenAI + Bedrock + AgentCore + Kiro + Prompt Engineering — exactly what AWS will want to feature.
- **High breadth / library-grade reuse:** every team deploying a Bedrock agent needs this; it even hardens the *outputs* of the other RAG/agent submissions.
- **Perfect meta-fit:** a prompt-engineering challenge rewarding a prompt that proves AI agents are safe.
- **Clear differentiation:** existing entries treat injection/least-privilege as sub-features; this is a standalone, OWASP-LLM-complete, evidence-recording, deploy-blocking system.
